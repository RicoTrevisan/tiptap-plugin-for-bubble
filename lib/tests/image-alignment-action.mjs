...
for (const alignment of ["left", "center", "right"]) {
    ...
    const selectedImage = imageEntry(harness.instance.data.editor);
    const currentImageDOM = harness.instance.data.editor.view.nodeDOM(selectedImage.pos);
    assert.ok(
        harness.instance.data.editor.storage.resizable.resizeElement === currentImageDOM,
        "resizer must track the current selected image DOM after alignment transactions",
    );
    assert.ok(
        harness.instance.data.editor.storage.resizable.resizeElement.isConnected,
        "resizer must not retain a detached image DOM node",
    );
    assert.equal(selectedImage.node.attrs.textAlign, alignment);
