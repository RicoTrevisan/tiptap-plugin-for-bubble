// Bubble can invoke update without initialize after reconstructing an element.
// Keep setup here so rendering is safe on first update and after reset.
if (typeof instance.data.renderTableOfContents !== "function") {
    ...
    function parseItems(value) {
        ...
        return parsed.map((item, index) => {
            ...
            const sourceLevel = Number(item.level);
            if (!Number.isInteger(sourceLevel) || sourceLevel < 1 || sourceLevel > 6) {
                throw new Error(`${path} level must be an integer from 1 to 6.`);
            }
            while (outlineLevels.length && sourceLevel < outlineLevels[outlineLevels.length - 1]) {
                outlineLevels.pop();
            }
            if (!outlineLevels.length || sourceLevel > outlineLevels[outlineLevels.length - 1]) {
                outlineLevels.push(sourceLevel);
            }
            return {
                id,
                textContent: item.textContent,
                level: outlineLevels.length,
